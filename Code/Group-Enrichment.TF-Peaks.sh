...
#Group-Enrichment.TF-Peaks.sh $rankedList $tfDir $tfExp $tfPeak $outputDir
...
awk 'function abs(x){return ((x < 0.0) ? -x : x)} \
    ...
    else sum = sum-1/(totalN-groupN); if (sum > max) max=sum}END{print max}' tmp.sort > $outputDir/$key.Group.txt
